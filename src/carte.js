...
export async function chargerElection(map, election, mode = "zone") {
  ...
  if (!cache[cle]) {
    ...
    augmenterLesResultatsAvecLesDonneesGeometrique(
      indexParBureauDeVote,
      donneesGeometrique,
    );
...
function augmenterLesResultatsAvecLesDonneesGeometrique(resultats, geometrie) {
  for (const lieu of geometrie) {
    if (lieu.bureaux) {
      const centre = lieu.geo_shape
        ? centroide(lieu.geo_shape)
        : lieu.geo_point_2d;
...
function fusionnerParCoordonnees(indexParBureauDeVote) {
  const indexFusionne = {};
  for (const [nomDeBureauDeVote, donneesDuBureauDeVote] of Object.entries(
    indexParBureauDeVote,
  )) {
...
function afficherLesDonnees(map, indexFusionneParCoordonnees, election, mode) {
  ...
  for (const { coordonnees, bureaux } of Object.values(
    indexFusionneParCoordonnees,
  )) {
    try {
      const creerMarqueur =
        mode === "point"
